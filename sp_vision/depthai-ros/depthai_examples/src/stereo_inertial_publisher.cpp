...
void expCallback(const std_msgs::Int32::ConstPtr& msg,std::shared_ptr<dai::DataInputQueue> controlQueue)//void chatterCallback(const std_msgs::String::ConstPtr& msg)
{
  ROS_INFO("I heard: [%d]", msg->data); 
  
  expTime = msg->data;
//   ROS_INFO("Now exptime: [%d]", expTime);
// //   dai::CameraControl ctrl;
    ctrl.setManualExposure(expTime, 800);
...
int main(int argc, char** argv) {
    ...
    ros::init(argc,argv,"exptime_listener");
    //3.实例化 ROS 句柄
    ros::NodeHandle nh_exptime;
    //4.实例化 订阅者 对象
    ros::Subscriber expTime_sub = 
        nh_exptime.subscribe<std_msgs::Int32>("expTime",10,
                                        boost::bind(&expCallback, _1, controlQueue));
// std::bind(&dai::rosBridge::ImageConverter::toRosMsg, &rgbConverter, std::placeholders::_1, std::placeholders::_2)
    ...
    if(manualExposure) {
        ...
        // ros::NodeHandle nh_exptime;
        // //4.实例化 订阅者 对象
        // ros::Subscriber expTime_sub = nh_exptime.subscribe<std_msgs::Int32>("expTime",10,expCallback);
        ROS_INFO("actual expTime: [%d]", expTime);
...
// void expCallback(const std_msgs::Int32::ConstPtr& msg,) //void chatterCallback(const std_msgs::String::ConstPtr& msg)
// {
//   ROS_INFO("I heard: [%d]", msg->data); 
