...
global R_gripper2base 
global T_gripper2base 
...
def find_silver_ore(color_image):
    ...
    hsv_image = cv2.cvtColor(color_image, cv2.COLOR_BGR2HSV)
    # 识别矿石的颜色特征
    ore_image = cv2.inRange(hsv_image, lower_silver, upper_silver)
    # 对识别后的图像进行腐蚀与膨胀，消除较小的连通域
    kernal = cv2.getStructuringElement(0, (3, 3))
    cv2.erode(ore_image, kernal, dst=ore_image)
    cv2.dilate(ore_image, kernal, dst=ore_image)
...
class ImageConverter:
    ...
    def detect_ore(self):
        ...
        frame = self.cv_image
        frame1 = self.cv_image
        ore_golden_box = find_golden_ore(frame)
            ...
            if len(contours) > 0:
                ...
                if len(point_array_n)>0:
                    ...
                    if suc:
                        ...
                        position = R_camera2gimbal * position + t_camera2gimbal
                        position = R_gripper2base *position + T_gripper2base
                        cx = position[0]
                        cy = position[1]
                        cz = position[2]
                        ...
                        print(rvec)
                        rvec= R_camera2gimbal * rvec
                        rvec= R_gripper2base * rvec
                        print(rvec)
        ...
        rospy.loginfo('运行时间为：{}ms'.format((end-start)*1000))
