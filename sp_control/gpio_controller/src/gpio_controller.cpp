...
namespace gpio_controller
{
  bool Controller::init(hardware_interface::RobotHW *robot_hw, ros::NodeHandle &root_nh, ros::NodeHandle &controller_nh)
  {
    ...
    for (int i = 0; i < xml_rpc_value.size(); ++i)
    {
      ...
      sp_control::GpioStateHandle state_handle_ = robot_hw->get<sp_control::GpioStateInterface>()->getHandle(gpioName);
      ...
      if (state_handle_.getType() == sp_control::OUTPUT)
      {
        sp_control::GpioCommandHandle command_handle_ =
            robot_hw->get<sp_control::GpioCommandInterface>()->getHandle(gpioName);
        gpio_command_handles_.push_back(command_handle_);
        ROS_INFO("Got command_gpio %s", gpioName.c_str());
      }
    }

    cmd_subscriber_ = controller_nh.subscribe<sp_common::GpioData>("command", 1, &Controller::setGpioCmd, this);
    return true;
  }
  ...
  void Controller::setGpioCmd(const sp_common::GpioDataConstPtr &msg)
  {
