...
namespace chassis_controller
{
    ...
    void SwerveWheel::moveJoint(const ros::Time &time, const ros::Duration &period)
    {
        ...
        para_xy = pow(vel_cmd_.x, 2) + pow(vel_cmd_.y, 2) + pow((half_wheel_base_/2) * vel_cmd_.z, 2) + pow((half_wheel_track_/2) * vel_cmd_.z, 2);
        para_x = half_wheel_track_/2  * vel_cmd_.z * vel_cmd_.x; 
        para_y = half_wheel_base_/2 * vel_cmd_.z * vel_cmd_.y;

        //遥控器无值时直接维持上次的值
        if (vel_cmd_.x==0 && vel_cmd_.y==0 && vel_cmd_.z==0)
                return;

	float test__ = -sqrt(para_xy - para_x + para_y)/wheel_radius_;
        //注意电机安装方向的不同
    ...
    geometry_msgs::Twist SwerveWheel::forwardKinematics()
    {
        ...
        vel_data.linear.x   = (rb_velocity*cos(rb_angle)+lf_velocity*cos(lf_angle))/2;
        vel_data.linear.y   = (rb_velocity*sin(rb_angle)+lf_velocity*sin(lf_angle))/2;
        vel_data.angular.z  = (rb_velocity*cos(rb_angle)-lf_velocity*cos(lf_angle))/(2*wheel_track_);
        return vel_data;
    }
